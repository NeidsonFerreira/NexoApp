...
export default function Mapa() {
  ...
  useEffect(() => {
    ...
    const precisaProfissional =
      contaAtual?.tipo === "cliente" && !!profissionalIdAtivo;
  ...
  useEffect(() => {
    localAtualRef.current = localAtual;
  }, [localAtual]);

  useEffect(() => {
    profissionalSelecionadoRef.current = profissionalSelecionado;
  }, [profissionalSelecionado]);
  ...
  useEffect(() => {
    seguindoCameraRef.current = seguindoCamera;
  }, [seguindoCamera]);
  ...
  async function buscarRotaNoApp(
    ...
    try {
      ...
      const stepsExtraidos: RotaStep[] = legs.flatMap((leg: any) =>
        Array.isArray(leg?.steps)
          ? leg.steps
              .map((step: any) => {
                const instruction = limparTextoInstrucao(step?.html_instructions || "");
      ...
      if (mapRef.current && coords.length > 1 && podeAtualizarCamera(1200)) {
