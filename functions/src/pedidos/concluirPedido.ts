...
export const concluirPedido = onCall({ region: REGION }, async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (pedido.profissionalId !== uid && pedido.clienteId !== uid) {
      throw new HttpsError("permission-denied", "Sem permissão.");
    }

    validarTransicaoStatus(pedido.status, "concluido");
