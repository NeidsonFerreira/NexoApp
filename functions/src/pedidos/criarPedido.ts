...
export const criarPedido = onCall({ region: REGION }, async (request) => {
  ...
  const profissionalId = requireString(request.data?.profissionalId, "profissionalId", 120);
  const tipoAtendimento = requireString(request.data?.tipoAtendimento, "tipoAtendimento", 80);
  const nomeCliente = requireString(request.data?.nomeCliente, "nomeCliente", 120);
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (profissional.tipo !== "profissional" && profissional.tipo !== "admin") {
      throw new HttpsError("failed-precondition", "Usuário selecionado não é profissional.");
    }

    if (!profissional.disponivel && !profissional.online) {
      throw new HttpsError("failed-precondition", "Profissional não está online.");
    }

    if (profissional.emAtendimento) {
      throw new HttpsError("failed-precondition", "Profissional já está em atendimento.");
    }

    if (cliente.emAtendimento || cliente.pedidoAtivoId) {
      throw new HttpsError("failed-precondition", "Cliente já possui pedido ativo.");
    }
